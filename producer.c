 ...
 * (semáforos y memoria compartida) creados en el sistema operativo antes de
 * salir.
 */
...
int main() {
  // Registrar los manejadores de señales para garantizar una limpieza segura en
  // caso de abortar
  signal(SIGINT, limpiar_recursos);
  signal(SIGTERM, limpiar_recursos);

  // 1. Limpiar recursos de ejecuciones anteriores para evitar corrupción de
  // datos
  shm_unlink("/memoria_compartida");
  sem_unlink("/vacio");
  sem_unlink("/lleno");
  ...
  // Abrir o crear el objeto de memoria compartida "/memoria_compartida" con
  ...
  // Ajustar el tamaño del objeto de memoria compartida al tamaño de nuestra
  // estructura compartir_datos
  ftruncate(shm_fd, sizeof(compartir_datos));

  // Mapear el objeto de memoria compartida en el espacio de direcciones de este
  // proceso
  compartir_datos *compartir =
      mmap(NULL, sizeof(compartir_datos), PROT_READ | PROT_WRITE, MAP_SHARED,
           shm_fd, 0);
  ...
  // 3. Inicializar a cero toda la estructura compartida (resetea terminado,
  // entrada, salida, bus) Esto asegura que la comunicación comience con un
  // estado perfectamente predecible
  memset(compartir, 0, sizeof(compartir_datos));

  // 4. Crear semáforos SOLO después de que la memoria compartida esté lista y
  // limpia. Esto garantiza que el consumidor no empiece a leer datos viejos
  // (estado persistente). Semáforo 'vacio' se inicializa con el tamaño total
  // del BUFFER (ranuras disponibles) Semáforo 'lleno' se inicializa con 0
  // (ranuras con elementos listos)
  ...
  for (int i = 1; i <= 10; i++) {
    ...
    // Escribir el elemento producido en la posición 'entrada' del búfer
    // circular
  ...
  // Despertar al consumidor en caso de que esté bloqueado esperando datos en
  // sem_wait(lleno)
  sem_post(lleno);

  printf("[Productor] Producción finalizada. Liberando memoria local...\n");

  // Liberar el mapeo de memoria y cerrar descriptores de archivos/semáforos del
  // proceso local
